...
class MAXAIApp(App):
    def build(self):
        ...
        def mkbtn(emoji, cb):
            b = Button(text=emoji, size_hint=(None,None), size=(dp(40),dp(44)),
                       background_normal='', background_color=(0.95,0.95,0.95,1),
                       color=(0.2,0.2,0.2,1), font_size=dp(20))
            b.bind(on_press=cb)
            return b

        btn_row.add_widget(mkbtn('📎', self.on_attach))
        btn_row.add_widget(mkbtn('📷', self.on_photo))
        btn_row.add_widget(mkbtn('🎥', self.on_video))
        btn_row.add_widget(mkbtn('🎤', self.on_mic))

        self.txt = TextInput(hint_text='Сообщение...', multiline=False,
                             size_hint=(1,None), height=dp(44), font_size=dp(16),
                             background_normal='', background_active='',
                             background_color=(0.97,0.97,0.97,1),
                             foreground_color=(0.1,0.1,0.1,1),
                             cursor_color=(0.24,0.52,0.98,1),
                             padding=[dp(10),dp(10)])
        self.txt.bind(on_text_validate=self.send_message)
        btn_row.add_widget(self.txt)

        send = Button(text='➤', size_hint=(None,None), size=(dp(44),dp(44)),
                      background_normal='', background_color=(0.24,0.52,0.98,1),
        ...
        Clock.schedule_once(lambda dt: self.add_message('Привет! Я MAX AI. Задай вопрос или прикрепи файл.', False), 0.4)
        return root

    def on_attach(self, *a): self.add_message('📎 Прикрепление файлов — в разработке', False)
    def on_photo(self, *a): self.add_message('📷 Камера — в разработке', False)
    def on_video(self, *a): self.add_message('🎥 Видео — в разработке', False)
    def on_mic(self, *a): self.add_message('🎤 Голос — в разработке', False)
    ...
    def send_message(self, *a):
        ...
        self._typing_row = MessageRow(text='...', is_user=False)
        self.msg_box.add_widget(self._typing_row)
    ...
    def _api(self, text):
        try:
            ...
            reply = r.json().get('response','Нет ответа')
        except Exception as e:
            reply = f'Ошибка: {e}'
        Clock.schedule_once(lambda dt: self._show_reply(reply))

    def _show_reply(self, text):
        if hasattr(self,'_typing_row') and self._typing_row in self.msg_box.children:
            self.msg_box.remove_widget(self._typing_row)
